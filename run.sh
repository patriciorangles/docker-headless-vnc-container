...
mostrar_ayuda() {
    ...
    echo
    echo "Opciones:"
    echo "  -t=, --tag=            Tag de la imagen Docker (por defecto: ${image_tag})"
    echo "  -c=, --contenedor=     Nombre del contenedor (por defecto: ${contenedor})"
    echo "  -p=, --port=           Puerto para acceso web (por defecto: ${port})"
    echo "  -v=, --VNCPASSW=       Contraseña VNC (por defecto: ${VNCPASSW})"
    echo "  -x=, --EXTRA_PARAM=    Parámetros extra para Docker (usados para el lanzamiento del contenedor)"
...
# Función para analizar parámetros con valor
parse_param() {
    local param="$1" default="$2"
    echo "${param#*=}" | grep -qE '^--?[a-zA-Z]' && echo "$default" || echo "${param#*=}"
}

# Procesar parámetros
for i in "$@"; do
    case $i in
        -t=*|--tag=*)        image_tag=$(parse_param "$i" "$image_tag") ;;
        -c=*|--contenedor=*) contenedor=$(parse_param "$i" "$contenedor") ;;
        -p=*|--port=*)       port=$(parse_param "$i" "$port") ;;
        -v=*|--VNCPASSW=*)   VNCPASSW=$(parse_param "$i" "$VNCPASSW") ;;
        -x=*|--EXTRA_PARAM=*) EXTRAPARAM=$(parse_param "$i" "$EXTRAPARAM") ;;
        -d|--delete)         delete="SI" ;;
